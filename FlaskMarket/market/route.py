...
for user in users:
    print_g('user.id', 'user.username', 'user.email_address', 'user.items', 'user.budget')

    
# END Play ground

# To refresh the server first, before routing to pages
def snatch(item):
    Item.query.filter_by(id=item.id).delete

def return_ownerless_items():
    items = Item.query.filter_by().all()
